...
function AmenitiesFirstCard(props: Props) {
    ...
    const partButton = () => {
        if (buttonCheck === false) {
            props.setFirstMenu([...props.firstMenu, props.data.text]);
            setBorderColor("black"); 
            setBorderCheck(true);
            setBorder(3);
            setButtonCheck(true);
        } else {
            for(var i = 0; i < props.firstMenu.length; i++){ 
                if (props.firstMenu[i] === props.data.text) { 
                  props.firstMenu.splice(i, 1); 
                  i--; 
                }
            }
            ...
            props.setFirstMenu(props.firstMenu);
        }
    };
...
export default AmenitiesFirstCard;